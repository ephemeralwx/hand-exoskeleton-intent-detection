...
ran `python online_learning_cnn_lstm.py --mode both` on 2026-03-30. 16 min training time on M2 Pro CPU.

3 seeds for randomized, 1 for chrono (deterministic so more seeds would be redundant).

## summary — randomized mode (3 seeds)

this is the main result table. "base" is the frozen loso model with no adaptation. "online" is after the online learner runs through the test set adapting as it goes. q4 = last 25% of the data, where the model has had the most updates, which should be closer to what real deployment performance would look like.
...
det% in this case = fraction of hand opening instances where at least 1 window was correctly predicted as positive.
